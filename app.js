...
router.post("/users", async (req, res) => {
  try {
    const { nickname, email, password, confirmPassword } = await postUsersSchema.validateAsync(req.body); //validateAsync는 에러가 날 경우 throw를 하기때문에 try catch로 감쌈
...
router.post("/auth", async (req, res) => {
    try{
      ...
      if (!user) {
        res.status(400).send({
          errorMessage: "이메일 또는 패스워드 형식이 잘못되었습니다.",
        });
        return;
      }
